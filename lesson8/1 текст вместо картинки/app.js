...
function hideMoreText(card) {
    ...
    let descEl = card.wrap.querySelector('.desc');
    descEl.remove();
...
function showMoreText(card) {
    ...
    let textForReplace = 'Подробная информация вместо картинки';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('beforeend', `<div class = "desc">${textForReplace}</div>`);
